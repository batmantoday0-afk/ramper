...
async function handleAccountSelection(interaction, autocatchers) {
  ...
  if (!selectedAc || !selectedAc.client.user) {
    return interaction.reply({ content: "❌ Selected account is no longer available!", flags: 64 });
  }

  if (!selectedAc.client.ws || selectedAc.client.ws.status !== 0) {
    return interaction.reply({ content: "❌ Selected account is offline!", flags: 64 });
  }
  ...
  if (validGuilds.length === 0) {
    return interaction.reply({ content: "❌ No suitable servers found with PokéTwo bot!", flags: 64 });
  }
...
async function handleMarketPurchase(interaction, autocatchers) {
  ...
  if (interaction.user.id !== userId) {
    return interaction.reply({ content: "❌ You cannot use this modal!", flags: 64 });
  }
  ...
  if (!selectedAc || !selectedGuild) {
    return interaction.reply({ content: "❌ Account or server is no longer available!", flags: 64 });
  }
