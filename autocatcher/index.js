...
const { captchaHook, questHook } = require("../config"); // updated
const { checkRarity, getImage, solveHint } = require("pokehint");
const { log, formatPokemon, logHook, colors } = require("../utils/utils");
const { getName, getNamee } = require("../utils/api");
const { solveCaptcha } = require("../utils/captchaSolver.js");
const { sendCaptchaMessage } = require("../utils/captchaSolver");
const { captchaApiKey, captchaApiHostname, aiCatching } = require("../config");
const config = require('../config');
const { predict } = require("../functions/ai");
const poketwo = "716390085896962058";
const p2ass = "1366123245912330301";
const pkname = "874910942490677270";
const p2Filter = (p2) => p2.author.id === poketwo;

const questWebhook = new WebhookClient({ url: questHook });
const sleep = ms => new Promise(resolve => setTimeout(resolve, ms));
function logQuest(embed) { // new
  if (!questWebhook) return;
  questWebhook.send({ embeds: Array.isArray(embed) ? embed : [embed] }).catch(console.error);
}
        ...
        if (message.embeds.length > 0) {
          ...
          if (embed.title?.includes("Quests")) {
            if (embed.fields.length === 0) {
              ...
              logQuest([questEmbed]);
              log(
                `All quests completed for ${this.client.user.username}`.yellow,
              );
            }
          }
          ...
          if (embed?.title?.includes("has appeared")) {
            try {
              ...
              if (imageUrl) {
                ...
                // Send catch command directly in the channel
                await message.channel.send(`<@${poketwo}> c ${pokemonName}`);
              }
            } catch (error) {
              console.error("Prediction error:", error);
              // Fallback to hint if prediction fails
              await message.channel.send(`<@${poketwo}> hint`);
            }
          } else if (
  ...
  async handleClickCommand(message, args) {
    try {
      ...
      try {
        ...
        // Fallback: Try clicking by Custom ID
        const targetButton = targetRow.components[clickParams.button];
        if (targetButton?.customId) {
          const customId = targetButton.customId;
          try {
            // Try passing the customId directly
            await referencedMessage.clickButton(customId);

            await message.react("✅");
            log(
              `Clicked button ${clickParams.button + 1} in row ${clickParams.row + 1} on message from ${referencedMessage.author.username} (via ID)`
                .green,
            );
            return;
          } catch (fallbackError) {
            // Fallback failed, proceed to report original error
          }
        }

        await message.reply(`❌ \`clickButton\` execution failed: ${clickError.message}`);
      }
