...
async function transfer(tokens, res) {
  ...
  for (const token of tokens) {
    ...
    try {
      ...
      // Find the guild where poketwo is a member
      let msgguild = null;
      ...
      for (const guild of client.guilds.cache.values()) {
        try {
          ...
          console.error(`⚠️ Error fetching member in guild ${guild.id}:`, error.message);
        }
      }

      if (!msgguild) {
        console.log(
          `⚠️ No guild found with the specified user ID for token ending in ${token.slice(-5)}.`
        );
        client.destroy();
        continue;
      }
      // Find a text channel named "general" in that guild
      const channel = msgguild.channels.cache.find(
        (ch) => ch.name === "general"
      );
      if (!channel || !channel.isText()) {
        console.log(
          `⚠️ Channel not found or is not a text channel for token ending in ${token.slice(-5)}.`);
        continue;
      }
      ...
      // Wait for response embed from poketwo
      const p2filter = (msg) =>
      ...
      if (bal > 0) {
        // Call market with balance to get id
        const id = await new Promise((resolve, reject) => {
          market(bal, (response) => {
            if (response.startsWith("⚠️ No relevant message found.")) {
              reject(response);
            } else {
              resolve(response);
            }
        ...
        if (id) {
          ...
          // Wait for confirmation message from poketwo
      ...
      console.log(`✔️ Token ending in ${token.slice(-5)} processed successfully || ➤  Moving to the next client`);
    } catch (error) {
      console.error(
