    ...
    tokenz.map(async (token) => {
      ...
      try {
        ...
        const timestamp = `[${now
          .toTimeString()
          .split(" ")[0]}]`; // HH:MM:SS
        return `${timestamp} Logged in as ${ac.client.user.username} ✅ (token ending in ${token.slice(
          -5
        )})`;
      } catch (error) {
...
async function addToken(token, callback) {
  ...
  try {
    ...
    setTimeout(() => {
      if (!loggedIn) {
        callback(
          `- Failed to login into ${token.substring(
            0,
            token.indexOf(".")
          ) || `_token_`} | Invalid Token?`.red,
          false
        );
      }
    }, 5000);
...
async function statMsg(message, page = 0) {
  ...
  const fields = autocatchers
    ...
    .map((x, i) => {
      ...
      const userPing = `<t:${Math.floor(x.stats.lastCatch / 1000)}:R>${x.captcha
  ...
  const activeConnections = autocatchers.filter(
    (x) => x.client.ws.status === 0
  ).length;
  const embed = new EmbedBuilder()
  ...
  const row1 = new ActionRowBuilder().setComponents(
    new ButtonBuilder()
      .setCustomId(
        `statPage-L-${page}-${message.author ? message.author.id : message.user.id
        }`
      )
      .setLabel("◀ Previous")
      .setDisabled(page === 0)
    ...
    new ButtonBuilder()
      .setCustomId(
        `statPage-R-${page}-${message.author ? message.author.id : message.user.id
        }`
      )
      .setLabel("Next ▶")
...
process.on("unhandledRejection", (error) => {
  ...
  crashCounter++;
  console.log(
    `Unhandled Promise Rejection caught (${crashCounter}):`,
    error.message
  );
